...
from mprocessor.models import Pipeline, Process, ProcessTarget, TriggerEvent
from mprocessor.make_plugins import pipeline, pipeline2, simple_pipe
from workspace.models import DAMWorkspace
from core.dam_repository.models import Type
from repository.models import Item, get_storage_file_name
from variants.models import Variant
from upload.views import _create_items, _run_pipelines
...
class DoTest:
    ...
    def register(self, name, trigger, media_types, description, pipeline_definition):
        ...
        pipe = Pipeline.objects.create(name=name.replace('_', ' '), description='',
                                       params=simplejson.dumps(pipeline_definition), workspace=self.ws)
        pipe.triggers.add(e)
        ...
        pipe.save
        print 'registered pipeline %s, pk=%s, trigger=%s' % (
        name, pipe.pk, '-'.join([x.name for x in pipe.triggers.all()]) )
